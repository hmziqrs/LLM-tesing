...
function RouteComponent() {
	...
	const transactionSummary = useQuery(orpc.transactions.getSummary.queryOptions());
	...
	if (accountsSummary.isLoading || budgetOverview.isLoading || transactionSummary.isLoading || upcomingBills.isLoading || goalsSummary.isLoading) {
		return <div className="p-6">Loading dashboard...</div>;
	}

	if (accountsSummary.error || budgetOverview.error || transactionSummary.error) {
	...
	return (
		<div className="p-6 space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							${parseFloat(budgetOverview.data?.totalSpent || "0").toFixed(2)} spent
						</p>
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							${parseFloat(transactionSummary.data?.income || "0").toFixed(2)} income,
							${parseFloat(transactionSummary.data?.expenses || "0").toFixed(2)} expenses
						</p>
					</CardContent>
				</Card>
			</div>
							...
							{goalsSummary.data && (
								<>
									<div className="flex justify-between text-sm">
										...
										<span>{goalsSummary.data.overallProgressPercentage.toFixed(1)}%</span>
									</div>
									<Progress value={goalsSummary.data.overallProgressPercentage} className="h-2" />
									<div className="flex justify-between text-xs text-muted-foreground">
										<span>${parseFloat(goalsSummary.data.totalSavedAmount || "0").toFixed(2)} saved</span>
										<span>${parseFloat(goalsSummary.data.totalTargetAmount || "0").toFixed(2)} target</span>
									</div>
								</>
							)}
							<Link to="/goals">
					...
					<CardContent>
						{upcomingBills.data && upcomingBills.data.length > 0 ? (
							...
							<div className="text-center py-4">
								<p className="text-muted-foreground text-sm mb-3">No upcoming bills</p>
								<Link to="/recurring">
			...
			<Card>
				...
				<CardContent>
					{budgetOverview.data?.categories && budgetOverview.data.categories.length > 0 ? (
						<div className="space-y-4">
							{budgetOverview.data.categories.slice(0, 5).map((category, index) => (
